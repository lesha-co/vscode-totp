...
exports.totpBackup = (context) => __awaiter(void 0, void 0, void 0, function* () {
    const getContent = (passphrase, data) => {
        if (passphrase === "") {
            return JSON.stringify({ cleartext: data });
        }
        else {
            return JSON.stringify({
                encrypted: context_1.encode(JSON.stringify(data), passphrase),
            });
        }
    };
    try {
        ...
        const content = getContent(passphrase, context_1.getCodes(context));
        const d = yield vscode_1.workspace.openTextDocument({ content });
        vscode_1.window.showTextDocument(d);
        vscode_1.window.showInformationMessage("Save this document somewhere and don't forge the passphrase!");
    }
    catch (x) {
        // debugger;
    }
...
exports.renameRoutine = (context, code) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    context_1.replaceCode(context, code.name, Object.assign(Object.assign({}, code), { name: newName }));
});
exports.totpEdit = (context) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const data = context_1.getCodes(context);
...
exports.totpPick = (context) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const ctxCodes = context_1.getCodes(context);
        ...
        if (result.button) {
            ...
            vscode_1.window.showInformationMessage("Password copied to clipboard");
        }
    }
    catch (x) {
        // debugger;
...
exports.totpRestore = (context) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (encrypted !== undefined) {
            passphrase = yield vscode_1.window.showInputBox({
            ...
            codes = [...codes, ...JSON.parse(context_1.decode(encrypted, passphrase))];
        }
        if (codes.length !== 0) {
            context_1.merge(context, codes);
            vscode_1.window.showInformationMessage(`${codes.length} passwords have been imported`);
        }
    }
    catch (x) {
...
exports.getCodes = (ctx) => {
    return ctx.globalState.get(KEY, []);
};
exports.setCodes = (ctx, codes) => {
    ctx.globalState.update(KEY, codes);
    ctx.globalState.update(STORE_VER_KEY, STORE_VER);
};
exports.addCode = (ctx, code) => {
    const allKeys = exports.getCodes(ctx);
    const newKeys = [code, ...allKeys];
    exports.setCodes(ctx, newKeys);
};
exports.replaceCode = (ctx, name, newCode) => {
    const allKeys = exports.getCodes(ctx);
...
exports.deleteCode = (ctx, name) => {
    const allKeys = exports.getCodes(ctx);
    const newKeys = allKeys.filter((key) => key.name !== name);
    exports.setCodes(ctx, newKeys);
};
exports.merge = (ctx, data) => {
    const ctxCodes = exports.getCodes(ctx);
    data.forEach((code) => {
